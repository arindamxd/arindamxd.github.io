...
- Vite 8 Rolldown: **do not** `optimizeDeps.include` React / `jsx-dev-runtime` (production CJS has `jsxDEV = undefined` → `_jsxDEV is not a function` in dev). Keep `react-activity-calendar` included; exclude the React entries. Alias `react/jsx-dev-runtime` to [`src/shims/react-jsx-dev-runtime.ts`](src/shims/react-jsx-dev-runtime.ts). `@astrojs/react` re-includes that id in `optimizeDeps` — strip it in the `configResolved` plugin after merge (a `config()` alias alone is overwritten).
